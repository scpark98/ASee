...
BOOL CASeeApp::InitInstance()
{
	...
	//shift 키를 누른 상태에서는 다중 실행을 허용한다.
	bool is_shift_pressed = IsShiftPressed();
	if ((::GetLastError() == ERROR_ALREADY_EXISTS) && !is_shift_pressed)
	{
		HWND	hWnd;
	...
	//CString str = GetProfileString(_T("setting"), _T("shell parameter"), _T(""));
	//AfxMessageBox(str);
