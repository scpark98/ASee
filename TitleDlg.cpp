...
BEGIN_MESSAGE_MAP(CTitleDlg, CDialog)
	...
	ON_REGISTERED_MESSAGE(Message_CSCSystemButtons, &CTitleDlg::on_message_CSCSystemButtons)
	//ON_WM_CREATE()
	ON_WM_TIMER()
	ON_WM_SIZE()
	ON_WM_PAINT()
...
BOOL CTitleDlg::OnInitDialog()
{
	...
	{
		m_hIcon = load_icon(NULL, IDR_MAINFRAME, 16, 16);// (HICON)::SendMessage(pMainWnd->GetSafeHwnd(), WM_GETICON, ICON_SMALL, 0);
	}
...
{
	CSCSystemButtonsMessage msg(NULL, SC_RESTORE);
...
void CTitleDlg::OnSize(UINT nType, int cx, int cy)
{
	...
	if (m_hWnd == NULL || m_sys_buttons.GetSafeHwnd() == NULL)
		return;

	CRect rc;

	GetClientRect(rc);
	m_sys_buttons.adjust(0, rc.right);
}

LRESULT CTitleDlg::on_message_CSCSystemButtons(WPARAM wParam, LPARAM lParam)
{
...
void CTitleDlg::OnPaint()
{
	...
	rTitle.right -= m_sys_buttons.get_size().cx;

	CString title;
	GetWindowText(title);

	CFont* pOldFont = (CFont*)dc.SelectObject(&m_titlebar_font);
	dc.DrawText(title, rTitle, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOCLIP);
